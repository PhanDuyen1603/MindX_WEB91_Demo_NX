...
// 0. Viết một API lấy apiKey để cấp quyền truy cập cho các API phía dưới
// GET /customers/getApikey/:id
// Kết quả trả về api key có dạng: web-${customerId}$-${email}-${randomString}$
// Ví dụ:  web-$12312321312$-$example@gmail.com$-$abcdef$
// randomString là chuỗi để xác thực api key và thuộc của một customer
// id: là id của customer
 ...
 export const getApikey = asyncHandler(async (req, res) => {
    ...
    if(!customer.apiKeyRand) {
        customer.apiKeyRand = crypto.randomBytes(6).toString('hex');
        await customer.save();
    }
